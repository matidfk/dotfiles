...
if [ -f /tmp/.google_drive_lock ]; then
    ...
    rclone copy /mnt/google_drive googledrive: --fast-list --update --progress --verbose "$@" 2>&1
    # notify-send "Rclone" "$output" --app-name="Rclone" --icon=folder-google-drive
    notify-send "Rclone" "Google drive push finished" --app-name="Rclone" --icon=folder-google-drive
fi
